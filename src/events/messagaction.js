...
module.exports = (client) => {
  client.on('messageCreate', async (message) => {
    ...
    if (message.content.toLowerCase().includes('arlo')) {
      // Reacts to messages that contain "Arlo"
      message.react('👀')
    }

    if (message.content.toLowerCase().includes('arlo, pin this')) {
      // Pins the message that contains the command
      const ModRoles = [
        '✦ Quill Master',
        'Senior Editor',
        'Copy Editor',
      ...
      message.react('📌').then(() => message.pin())
    }

    if (message.content.toLowerCase().includes('arlo, pin it')) {
      // Pins the message that the command message is replying to
      const ModRoles = [
        '✦ Quill Master',
        'Senior Editor',
        'Copy Editor',
      ...
      if (!message.reference)
        return message.reply('Reply to a message for me to pin it.')
    ...
    if (message.content.toLowerCase().includes('arlo, start knitting')) {
      // Starts a thread with the message that contains the command
      const ModRoles = [
        '✦ Quill Master',
        'Senior Editor',
        'Copy Editor',
...
I request <@&1479068137616834754>, <@&1479068546054230016>, <@&1479069863103172770>, <@&1479070225361272884>, <@&1479070290800545944>, <@&1479070354264821770> to join the thread to assist with the request.`)
    ...
    if (message.channelId === process.env.INTRODUCTION_CHANNEL_ID) {
      // Reacts to messages in #🖋️・introduction channel
      message.react('👋🏻')
    }

    if (message.channelId === process.env.MILESTONES_CHANNEL_ID) {
      // Reacts to messages in #🎉・milestones channel
      message.react('👏🏻').then(() => message.react('🎉'))
    }

    // Embeds sent through request in specific channels | **Used rarely**

    if (message.content.toLowerCase().includes('arlo, update guide')) {
      // Embed for the 📖・guide channel
      ...
      const GuideWelcImgEmbed = new EmbedBuilder() // Welcome Image Embed for the 📖・guide channel
        .setColor(`#B0E4CC`)
        .setImage(
          `https://media.discordapp.net/attachments/1180848523051270216/1488082590903566426/Welcome_Embed.png?ex=69cb7c68&is=69ca2ae8&hm=9ba68cb684f36cab61073ff2f79d44f228522269979525b4df6712b298f1ee89&=&format=webp&quality=lossless`,
        )

      const GuideWelcEmbed = new EmbedBuilder() // Welcome Embed for the 📖・guide channel
        .setTitle('🕯️ Welcome to Nova Archives')
        .setDescription(
          `You've just walked into a space built for people who hear voices in their heads and call it writing.
...
The blank page is less terrifying with people around it. Now go write something.`,
      ...
      const GuideRulesImgEmbed = new EmbedBuilder() // Rules Image Embed for the 📖・guide channel
        .setColor(`#B0E4CC`)
        .setImage(
          `https://media.discordapp.net/attachments/1180848523051270216/1488082589142220912/Rules_Embed.png?ex=69cb7c68&is=69ca2ae8&hm=bebe6c0d74fa6650045a4a08ac9ef896a8a10353e683465565cca02f9f7e69b4&=&format=webp&quality=lossless`,
        )

      const GuideRulesEmbed = new EmbedBuilder() // Rules Embed for the 📖・guide channel
        .setTitle('📜 The Codex — Server Rules')
        .setDescription(
          `These aren't restrictions. They're the reason this place stays worth being in.`,
        )
        .setFields(
          {
            name: 'Conduct',
...
14. Share your Wattpad, AO3, Substack, blog in <#1479413971181240390>. One post per author, kept updated`,
      ...
      const GuideSerChaImgEmbed = new EmbedBuilder() // Server Categories Image Embed for the 📖・guide channel
        .setColor(`#B0E4CC`)
        .setImage(
          `https://media.discordapp.net/attachments/1180848523051270216/1488082589540548700/Server_Categories_Embed.png?ex=69cb7c68&is=69ca2ae8&hm=c3366b7aeac03674f3fc709e7d2315fd1d3d51898c1cdf17f59c563d9d1cce99&=&format=webp&quality=lossless`,
        )
      ...
      const GuideSerChaEmbed = new EmbedBuilder() // Server Categories Embed for the 📖・guide channel
        ...
        .setDescription(
          `A lot of channels. Not a single one is filler. Here's what lives where.`,
        )
        .setFields(
          {
      ...
      const GuideTeamImgEmbed = new EmbedBuilder() // Team Image Embed for the 📖・guide channel
        .setColor(`#B0E4CC`)
        .setImage(
          `https://media.discordapp.net/attachments/1180848523051270216/1488082589884616857/Team_Embed.png?ex=69cb7c68&is=69ca2ae8&hm=c69ca445a4d0eada4bc4b349dc4d79e509765527567921dae3e667e74748101d&=&format=webp&quality=lossless`,
        )

      const GuideTeamEmbed = new EmbedBuilder() // Welcome Embed for the 📖・guide channel
        .setTitle('🗝️ The Editors — Meet the Team')
        .setDescription(
          `Nova Archives runs because real people keep the lights on.
...
Moderation won't be able to handle issues through member-to-member DMs — if something goes wrong in a DM conversation with another member, report it directly to Discord's Trust & Safety.`,
      ...
      const GuideChannel = client.channels.cache.get(
        process.env.GUIDE_CHANNEL_ID,
      )
    ...
    if (message.content.toLowerCase().includes('arlo, update staff')) {
      // Embed for the 📋・staff-info channel
      ...
      const StaffInfoImgEmbed = new EmbedBuilder() // Staff Info Image Embed for the 📋・staff-info channel
        .setColor(`#B0E4CC`)
        .setImage(
          `https://media.discordapp.net/attachments/1180848523051270216/1488082590350049390/Team_Info_Embed.png?ex=69cb7c68&is=69ca2ae8&hm=6a2b34ffa2bf6553ae0473ffdb506760cf4b13f66df973774a876bec4c9838f0&=&format=webp&quality=lossless`,
        )
      ...
      const StaffInfoEmbed = new EmbedBuilder() // Staff Info Embed for the 📋・staff-info channel
        ...
        .setDescription(
          `These are the people who keep Nova Archives running. Each role carries specific responsibilities and a specific level of trust. If you're reading this as a new staff member, this is who you are now and what that means.`,
        )
        .setColor('#285A48')
        .setFields(
          {
        ...
        .setFooter({
          text: 'Roles are earned. Permissions are a tool, not a reward.',
        })

      const StaffDutiesImgEmbed = new EmbedBuilder() // Staff Duties Image Embed for the 📋・staff-info channel
        .setColor(`#B0E4CC`)
        .setImage(
          `https://media.discordapp.net/attachments/1180848523051270216/1488164813170409722/Team_Disc_Embed.png?ex=69cbc8fc&is=69ca777c&hm=32eba991e1681002ed3f43fff6d241a93ee9daef471fc08daa6da45e75b20fd2&=&format=webp&quality=lossless`,
        )
...
- Show up consistently in the community spaces — Workshop, Genre Dens, Lounge. Your presence is the job.`,
      ...
      const StaffDutiesBEmbed = new EmbedBuilder() // Staff Duties Embed for the 📋・staff-info channel
        ...
        .setDescription(
          `Regardless of role, every staff member in Nova Archives is expected to uphold the following.`,
        )
        .setColor('#285A48')
        .setFields(
          {
...
Claim the 'Shelved Editor' role from the button below to signal when you're taking a break from staff duties.`,
        ...
        .setFooter({
          text: 'The server is only as good as the people running it.',
        })
      ...
      const StaffCommandsEmbed = new EmbedBuilder() // Staff Commands Embed for the 📋・staff-info channel
        ...
        .setDescription(
          `Arlo handles the heavy lifting so moderation stays consistent and logged. Here's what you have access to and how to use it.`,
        )
        .setColor('#285A48')
        .setFields(
          {
...
Once a decision is reached and action is taken, the staff member handling it posts a brief summary of what was done inside the thread, then closes the thread. Closed threads are archived, not deleted — the record stays.`,
        ...
        .setFooter({
          text: 'Arlo logs everything. The audit trail is always there.',
        })

      const StaffChannelsImgEmbed = new EmbedBuilder() // Staff Channels Embed for the 📋・staff-info channel
        .setColor(`#B0E4CC`)
        .setImage(
          `https://media.discordapp.net/attachments/1180848523051270216/1488181442214232114/Moderators_Channels_Embed.png?ex=69cbd878&is=69ca86f8&hm=d02da2d6a5337a6da0fd851a47d6e51d5009cbbe372f573e35a1b89d2a344d88&=&format=webp&quality=lossless`,
        )
      ...
      const StaffChannelsEmbed = new EmbedBuilder() // Staff Channels Embed for the 📋・staff-info channel
        ...
        .setDescription(
          `Staff Wing and Moderation are your private infrastructure. Here's exactly what each channel is for so nothing gets posted in the wrong place.`,
        )
        .setColor('#285A48')
        .setFields(
          {
...
<#${process.env.DEVLOGS_CHANNEL_ID}> — Arlo's changelog. Every update to the bot gets posted here by the Quill Master`,
        ...
        .setFooter({
          text: `If something doesn't have a home, it goes in #staff-lounge until it does.`,
        })

      const ShelvingEmbed = new EmbedBuilder()
        .setTitle('🔴 Toggle your availability status. 🟢')
        .setDescription(
          `Click the button below to toggle your <@&1493154104078241792> role. This signals to the rest of the team that you're taking a break from staff duties.
You can still be active in the server, but you won't be pinged for moderation or event tasks while shelved.`,
        )
        .setColor('#B0E4CC')

      const ShelvedRow = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
      ...
      const StaffInfoChannel = client.channels.cache.get(
        process.env.STAFFINFO_CHANNEL_ID,
      )
      await StaffInfoChannel.send({
        embeds: [
          StaffInfoImgEmbed,
          StaffInfoEmbed,
          StaffDutiesImgEmbed,
      ...
      await message.channel.send({
        embeds: [ShelvingEmbed],
        components: [ShelvedRow],
      })
      message.react('🫡')
    }

    if (message.content.toLowerCase().includes('arlo, reload the gun')) {
      // Embeds for the 🎲・prompt-roulette channel
      const ModRoles = ['✦ Quill Master', 'Senior Editor']
      const hasRole = message.member.roles.cache.some((r) =>
        ModRoles.includes(r.name),
      )
      ...
      const PromptRouletteWelcEmbed = new EmbedBuilder() // Welcome Embed for the 🎲・prompt-roulette channel
        ...
        .setDescription(
          `Here you can generate random writing prompts to spark your creativity. Whether you're looking for a quick idea or a new concept, Prompt Roulette has you covered.
...
Happy Writing!`,
      ...
      const FragBuildEmbed = new EmbedBuilder() // Fragment Builder Embed for 🎲・prompt-roulette
        ...
        .setDescription(
          `Build a story piece by piece using structured elements like character, setting, conflict, and theme.
Perfect for when you want direction without losing creative control.`,
      ...
      const ConsChaosEmbed = new EmbedBuilder() // Constraint Chaos Embed for 🎲・prompt-roulette
        ...
        .setDescription(
          `Generate unusual writing restrictions that force you to think differently and break привычные patterns.
Ideal for sharpening creativity by working within limits.`,
      ...
      const WildCardEmbed = new EmbedBuilder() // Wildcard Story Spark Embed for 🎲・prompt-roulette
        ...
        .setDescription(
          `Receive a completely random mix of elements like character, object, mood, and genre.
Designed for pure spontaneity and unpredictable inspiration.`,
      ...
      const PromptRouletteChannel = client.channels.cache.get(
        process.env.PROMPTROULETTE_CHANNEL_ID,
      )
      await PromptRouletteChannel.send({ embeds: [PromptRouletteWelcEmbed] })
